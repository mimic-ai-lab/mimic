...
export default function LoginPage() {
  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    setError(null);
    try {
      // Use the Better Auth client for magic link
      // According to the documentation, use signIn.magicLink
      await authClient.signIn.magicLink({
        email: data.email,
        callbackURL: '/', // Redirect to dashboard after successful login
      });
  ...
  const handleSocialAuth = (provider: 'github' | 'google') => {
    // You can implement social auth using authClient if configured
    // For now, just redirect to the provider's auth URL if needed
                ...
                >
                  ...
                  <div className="text-center space-y-2 mt-auto pt-4">
                    <p className="text-xs lg:text-sm text-gray-500">
                      © Mimic AI Lab. By signing in, you agree to our Terms of
